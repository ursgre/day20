	...
	public function action($type, $table, $id = '') {
		if(checkAccess($access_group = ['administrator', 'redaktor'], $_SESSION['rola'])) {
			...
			foreach ($_POST as $key => $value) {
				...
				if (!$this->db->field_exists('photo_2', $table)) {
					$this->base_m->create_column($table, 'photo_2');
				}
				...
				} elseif($key == 'name_photo_2') {
					if ($this->upload->do_upload('photo_2')) {
						$data = $this->upload->data();
						$insert['photo_2'] = $now.'/'.$data['file_name'];
						if($data['image_width'] > 1440) {
							resizeImg($data['file_name'], $now, '1440');
						}   
						if ($data['file_type'] != 'image/svg' && isOnWebp()) {
							webp_convert($insert['photo_2']); 
						}
						addMedia($data);
					} elseif($value == 'usunięte') {
						$insert['photo_2'] = '';
