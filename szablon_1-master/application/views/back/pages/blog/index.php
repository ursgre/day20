                ...
                <table id="datatable1" class="table display responsive nowrap">
                    <thead>
                        <tr>
                            ...
                            <th class="wd-5p align-top">Kolejność</th>
                            ...
                            <td class="align-middle">
                                <label class="ckbox">
                                    <input type="checkbox" id="check<?= $value->id ?>"
                                        onchange="setActiveRecord(<?= $value->id ?>);"
                                        <?php if (@$value->active) echo "checked"; ?>>
                                    <span></span>
                                </label>
                            </td>
                            <td class="align-middle">
                                <input class="form-control w-50" id="getOrder<?= $value->id ?>" name="record_order"
                                    onkeyup="changeOrder(<?= $value->id ?>)" value="<?= @$value->record_order ?>">
                            </td>
                            <td class="align-middle"><?php echo substr($value->title, 0, titleLength());
                                                            if (strlen($value->title) > titleLength()) echo '...' ?>
                            </td>
                            <td class="align-middle"><?php echo $value->created; ?></td>
                            <td class="text-right">
                                <a href="<?php echo base_url(); ?>panel/settings/gallery/<?php if ($this->session->userdata('lang') != 'pl') echo $this->session->userdata('lang') . '_' . $this->uri->segment(2);
                                                                                                else echo $this->uri->segment(2);  ?>/<?php echo $value->id; ?>"
